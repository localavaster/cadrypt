                              ...
                              for (int sentenceLength = 3;
                                ...
                                for (final page in solvedPages.listSync()) {
                                  ...
                                  pageWords.removeWhere((element) =>
                                      element.isEmpty || element == null);

                                  for (int i = 0; i < pageWords.length; i++) {
                                    final int offset = sentenceLength;
                                    final sentence = pageWords.sublist(
                                        i,
                                        (i + offset)
                                            .clamp(0, pageWords.length)
                                            .toInt());
                                ...
                                for (final sentence in sentences) {
                                  ...
                                  for (final page in pages) {
                                    ...
                                    final sentenceToFind = sentence.replaceAll(
                                        RegExp('[^ ]'), '[${runes.join()}]');

                                    final regexSentence =
                                        RegExp('([. ]$sentenceToFind[. ])');

                                    final matches = regexSentence.allMatches(
                                        pageBody.replaceAll(RegExp('\n'), ''));

                                    //print('parsing page: ${page_handle.path.split('/').last.allBefore('.')} matcher: $regexSentence. matches: ${matches.length}');

                                    matches.forEach((element) {
                                      sentenceMatches[sentence][pageHandle.path
                                          .split('/')
                                          .last
                                          .allBefore('.')] ??= [];
                                  ...
                                  if (sentenceMatches[sentence].isNotEmpty) {
                                    final sentenceEnglish =
                                        List<String>.generate(sentence.length,
                                            (index) {
                                      final character =
                                          sentence.characters.elementAt(index);
                                    ...
                                    sentenceMatches[sentence]
                                      ...
                                      final outputFile = File(
                                          '${Directory.current.path}/console_output/sentence_finder/${sentenceLength}_${pageName}_${sentenceEnglish.join()}');
